...
from life_expectancy.tests.aux_test_cleaning_abc_tsv import ExpectancyTestPipeline
from . import FIXTURES_DIR

class TsvTest(ExpectancyTestPipeline):
    """
    This class has the test functions
    in regards to the TSV ETL pipeline.
    """
    # create instance from ExpectancyTsv
    tsv_instance =  ExpectancyTsv()

    def test_load(self, life_expectancy_input_path_tsv):
        """
        Loads the data from the tsv file to a pandas dataframe
        """

        # load the data from the tsv
        df_loaded = self.tsv_instance.load_data(life_expectancy_input_path_tsv)
    ...
    def test_clean_data(self, life_expectancy_input_path_tsv, pt_life_expectancy_expected_tsv):
        ...
        # clean the data
        df_cleaned = self.tsv_instance.clean_data(df_loaded, region)

        pd.testing.assert_frame_equal(
            df_cleaned, pt_life_expectancy_expected_tsv
        )

    def test_save(self, life_expectancy_input_path_tsv, pt_life_expectancy_expected_tsv):
        ...
        with mock.patch('pandas.DataFrame.to_csv') as mock_to_csv:
            ...
            df_cleaned.to_csv(FIXTURES_DIR / "pt_life_expectancy_expected.csv", index=False)

            # path where the cleaned df will be stored
            out_path: Path  = FIXTURES_DIR / "pt_life_expectancy_expected.csv"

            # load the dataframe that was just cleaned and created
            pt_life_expectancy_actual = pd.read_csv(out_path)

            pd.testing.assert_frame_equal(
                pt_life_expectancy_actual, pt_life_expectancy_expected_tsv
            )
